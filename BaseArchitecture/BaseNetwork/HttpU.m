...
-(void)requestPost:(NSString *)urlPath withParameters:(NSDictionary *)param completedBlock:(void(^)(id responseObject))completedBlock failureBlock:(void(^)(NSError *error))failureBlock{
    ...
    NSString *version = [AppBaseTool getAppVersion];
    NSString *WXVersion= [[NSUserDefaults standardUserDefaults] objectForKey:@"oldWXVersion"];
    NSString *identifierForVendor = [AppBaseTool getDeviceIDFV];
    NSString *deviceName = [AppBaseTool deviceModelName];//获取设备的型号
    ...
    if ([[NetworkManager shareManager] isGetRequest:urlPath]) {
        
        if ([[NetworkManager shareManager] haveNetwork] && !self.isCache) {
